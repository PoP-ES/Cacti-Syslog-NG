...
function loadXMLDoc()
{
	...
	parameters += "&facility="+document.getElementById('facility').value;
	parameters += "&priority="+document.getElementById('priority').value;
	parameters += "&program="+document.getElementById('program').value;
...
<?php
print "<form name='form_events' id='frm' method=POST action='" . $config['url_path'] . "plugins/syslog/syslog.php' onsubmit='return checkform(this);'>";
...
print "<td >&nbsp;<strong>FACILITY:&nbsp;</strong>";
print '<select name="facility" id="facility" size="1">';
$facilities = array("","kern", "user", "mail", "daemon", "auth", "syslog", "lpr", "news", "uucp", "cron", "local0", "local1", "local2", "local3", "local4", "local5", "local6", "local7");
selectedOptions($set['sqlcond']['facility'], $facilities);
print "</td>";
print "<td >&nbsp;<strong>PRIORITY:&nbsp;</strong>";
print '<select name="priority" id="priority" size="1">';
$priorities = array("","emerg", "alert", "crit", "err", "warning", "notice", "info", "debug", "none");
selectedOptions($set['sqlcond']['priority'], $priorities);
print "</td>";
print "<td >&nbsp;<strong>PROGRAM:&nbsp;</strong>";
print '<input size=15 type="text" id="program" name="program" value="' . $set['sqlcond']['program'] . '" />';
print "</td>";
