...
$fields = preg_split('/[, ]/', read_config_option("syslog_fields"));
$set['sqlcond'] = array();

/* read Database configs */
$dbType = read_config_option("syslog_dbType");
$dbUser = read_config_option("syslog_dbUser");
$dbPass = read_config_option("syslog_dbPass");
$dbHost = read_config_option("syslog_dbHost");
$dbPort = read_config_option("syslog_dbPort");
$dbName = read_config_option("syslog_dbName");
...
$set['sqlcond']['facility'] = ( isset($_POST['facility']) ? $_POST['facility'] : "teste");
$set['sqlcond']['priority'] = ( isset($_POST['priority']) ? $_POST['priority'] : null);
$set['sqlcond']['program'] = ( isset($_POST['program']) ? $_POST['program'] : null);
$set ['total'] =  False;
...
function print_table() {
	...
	if ($total == 0)
		print "<tr bgcolor='$bg'><td style='padding: 4px; margin: 4px;' colspan=100%><center>There are no Hosts to display!</center></td></tr>";
